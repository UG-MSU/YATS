...
def update(request):
    if len(get_user_model().User.filter(username=request.data["username"])) == 0 and len(request.data["username"]) != 0:
    ...
    request.user.save()
    User = authenticate(request=request, username=request.user.username, password=request.data["password"])
    login(request, User)
...
class RegUserAPIView(generics.ListAPIView):
    ...
    def post(self, request):
        try:
            ...
            created_user = get_user_model().User.create_user(request.data["username"], 'none@none.none', request.data["password"])
            created_user.save()
            current_user = authenticate(request=request, username=request.data["username"], password=request.data["password"])
...
class UpdateUserAPIView(generics.ListAPIView):
    ...
    def put(selfself, request):
        if request.user is None:
            return Response({'error': 'user is not authenticated'})
        update(request)
        return Response({'error' : 'success', 'user': serializers.UpdateUserSerializer(request.user).data})
